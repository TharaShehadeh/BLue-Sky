...
public class URoundtripController implements Initializable {
    ...
    @FXML
        ...
        if ((flag1&&flag2&&flag3) )
         ...
         try {
            ...
            String dpart = this.dpart.getText();
            String arrive = this.arrive.getText();
            String departdate = this.departdate.getId();
            String returndate = this.returndate.getId();
            
            
            String str_st = "insert into roundTripsearch values( '"+dpart+"','"+arrive+"', '"+departdate+"', '"+returndate+"')" ;
            
            stm.executeUpdate(str_st);
        
         Parent tabelViewParent = FXMLLoader.load(getClass().getResource("flights.fxml"));//مكان ما بدي اروح
